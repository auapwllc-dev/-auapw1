...
const LogoCtx  = createContext<{logoSrc:string|null,setLogoSrc:(s:string|null)=>void}>({ logoSrc:null, setLogoSrc:()=>{} });
...
function SecHead({label,title,center=false,tok,cls="chrome"}){
  return(
...
function QuoteForm({defaultPart="",tok}){
  ...
  const [errs,setErrs]=useState<Record<string,string>>({});const [status,setStatus]=useState("idle");
  ...
  const sS=(err)=>({...iS(err),appearance:"none" as const,cursor:"pointer"});
            ...
            <div key={col.h}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:7}}>
                {col.items.map(function(x){return(
                  <button key={x.p+x.l} onClick={()=>goTo(x.p)} style={{background:"none",border:"none",textAlign:"left",fontSize:11,color:tok.muted,cursor:"pointer",fontFamily:MH,padding:0,transition:"color .2s",letterSpacing:".02em"}} onMouseEnter={e=>(e.target as HTMLElement).style.color=tok.fg} onMouseLeave={e=>(e.target as HTMLElement).style.color=tok.muted}>{x.l}</button>
                );})}
              </div>
            </div>
          );})}
        ...
        <div style={{padding:"12px 0",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {["privacy","terms","shipping","returns","cookies","disclaimer"].map(p=>(
              <button key={p} onClick={()=>goTo(p)} style={{background:"none",border:"none",fontSize:9,color:tok.dimmer,cursor:"pointer",fontFamily:MH,letterSpacing:".06em",padding:0,textTransform:"capitalize"}} onMouseEnter={e=>(e.target as HTMLElement).style.color=tok.fg} onMouseLeave={e=>(e.target as HTMLElement).style.color=tok.dimmer}>{p}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:1360,margin:"0 auto",position:"relative",zIndex:3}}>
        ...
        <div className="hg" style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 400px",gap:44,alignItems:"start"}}>
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            <p style={{fontSize:13,color:tok.muted,lineHeight:1.9,fontWeight:300}}>AUAPW.ORG is a US-based online platform for finding all kinds of used auto parts. We cooperate with the best junkyards and salvage yards around the country to provide our customers with only top-notch services. We help you connect with the best suppliers without extra effort — serving as a bridge between you and the dealers. After filling the part request form, you will receive a response in <strong style={{color:tok.fg}}>less than 24 hours</strong>. Taking care of your vehicle can sometimes be expensive, but with used auto parts you can save a fortune. We guarantee that you will get only <strong style={{color:tok.fg}}>high-quality products at the most affordable prices</strong>.</p>
...
function UsedEnginesPage({goTo,tok}){
  ...
  const faqs=[
    {q:"Are used engines good?",a:"Firstly, consider the money you are willing to spend. Used engines can be a great deal, but they may also come with some hidden costs. Second, consider the source. If you are buying from a reputable dealer like AUAPW.ORG, you are likely to get a quality product. Because used engines are rigorously examined before being sold, their reliability is far greater than most people expect."},
    {q:"Is a used engine worth buying?",a:"To begin, make sure the engine is compatible with your car. Second, check to see if the engine is in working order. Finally, make sure you are getting a reasonable price. Used engines can be a great option for anyone looking to save money on car maintenance. However, undertake extensive research to verify that you are choosing a high-quality, long-lasting engine. AUAPW.ORG does this research for you."},
    {q:"Which is better - rebuilt or used engine?",a:"Both used and remanufactured engines have advantages and disadvantages. Although a used engine is less expensive, it may have greater wear and tear. The cost of a rebuilt engine is higher, but it will last longer. If you are looking to save money, a used engine is the best choice. A rebuilt engine is a superior alternative if reliability is your primary priority."},
    {q:"Is replacing an engine worth it?",a:"Replacing your car engine is really worth your money and extends your vehicle life. Buying a used car engine is a wise investment that will have a significant impact on your financial cash flow. By choosing to buy a used engine, you avoid the production of a new engine, which is environmentally good. An engine replacement can result in savings of taxes, license fees, and insurance costs that would be incurred if you buy a new vehicle."},
    {q:"How many miles can an engine last?",a:"According to AARP, the average mileage of a car engine is assumed to be between 12,000 and 15,000 miles per year. Always adhere to the maintenance plan specified by the engine manufacturer. When you notice even small things from your engine, do not put them off. Ensure your engine is free from grime, dirt, sludge, and other contaminants. Abide by the speed limit to decrease wear and prolong engine lifespan."},
    {q:"What should I look for when buying a used engine?",a:"Look for the part with the right specifications of your existing engine. The more miles the car has, the more wear and tear it will have. Look for a warranty associated with your purchase. Before buying, take a few minutes to look up the seller credentials. Check for certifications, testimonials, and reviews. Ensure the engine has been tested for necessary performance parameters."},
    {q:"How do I find a good used engine?",a:"When considering used engines for sale, the trustworthiness of the seller is the most important factor. At AUAPW.ORG, we are connected with all junkyards and salvage yards across all US states. When you fill the part request form, our system sends the request to all auto part dealers and receives part availability with pricing. The quality engine and lowest price quote will be sent to you within 24 hours."}
  ];
  return <>
...
function UsedTransPage({goTo,tok}){
  ...
  const tenPoints=[
    {n:"1",t:"Check the Condition",d:"The foremost step is knowing the existing condition of the used transmission. If you do not know much about transmissions, bring a mechanic along while purchasing. They will look for the aspects needed for a transmission to be in great shape."},
    {n:"2",t:"Quality of Transmission Fluid",d:"Before deciding to buy, check the quality of transmission fluid. Locate the dipstick. Usually transmission fluid is red — if it has changed to brown, it needs replacing. If it is blackish, stay away — that color is due to overheating."},
    {n:"3",t:"Check for Fluid Leakage",d:"Pour some transmission fluid inside and observe the body of the transmission. Check whether there are any signs of liquid coming out. If you see any leakage, go with a different used transmission."},
    {n:"4",t:"Listen for Unusual Noise",d:"If the transmission makes grinding, popping, or squealing noises when you row through the gears, switch to another used transmission. You do not want to be stuck in the middle of driving when a piece of the transmission breaks loose."},
    {n:"5",t:"Is There a Warranty?",d:"Reputable shops offer a warranty with used transmissions. Typically most dealers offer a 12 month or 12,000 mile warranty. Try to get a warranty that covers both parts and labor. AUAPW.ORG includes a 30-180 day warranty on every transmission."},
    {n:"6",t:"Check Transmission Engagement",d:"Start the transmission in Park mode. It should start and idle without unnecessary noise. Then shift to Drive. The lever should shift smoothly. Any resistance or unusual behavior is a red flag."},
    {n:"7",t:"Have the Transmission Tested",d:"Technology makes it possible for sellers to conduct tests on used auto parts before installation. Make sure the specific part is tested for quality before completing the transaction to ensure you buy a quality transmission."},
    {n:"8",t:"Get the Maintenance Records",d:"Regular maintenance is essential for a quality transmission. Ensure the transmission was serviced every 20,000 to 50,000 miles per the owners manual. Check for any past repairs and confirm they were performed by certified mechanics."},
    {n:"9",t:"Check the Fluid Level",d:"Ensure the transmission fluid is filled to the manufacturer approved level with quality oil to keep internal parts moving freely. Changing fluid often keeps the transmission in perfect shape and avoids repeated repairs."},
    {n:"10",t:"Check References",d:"Any reputed seller can provide a list of customer references. Check with those references and ask about their buying experience. Ask specifically about customers who bought a similar model of used transmission in the past."}
  ];
  const faqs=[
    {q:"Is it better to buy a used or rebuilt transmission?",a:"The first concern is price. Although used transmissions are less expensive, they may not last as long as rebuilt transmissions. Rebuilding a transmission is more expensive, but may provide a longer-lasting solution. The source is the second factor — a used transmission from a reliable source like AUAPW.ORG will last longer than one from a less reputable source. However, even a reputable used transmission may not last as long as a rebuilt one. Consider your own mechanical abilities and budget when making this decision."},
    {q:"How can I get a cheap transmission?",a:"There are a few options for obtaining a low-cost transmission. One option is to purchase a used transmission from a salvage yard or individual. Another option is to find a transmission rebuilder that may save you money by repairing your existing transmission. Finally, you might negotiate a reduced price on a new transmission with your mechanic or dealership. Whatever option you choose, conduct thorough research and get multiple quotes before making a final decision. AUAPW.ORG searches 2,000+ yards to find the lowest price for you."},
    {q:"How can you get a warranty on a used transmission?",a:"One approach is to buy a used transmission from a salvage yard that offers guarantees on their used parts. Before you buy, check the warranty terms and conditions carefully. Another approach is to find a used transmission that still has the manufacturer warranty. Finally, a warranty from a third-party company may be available — always examine the terms to make sure it covers everything you need. AUAPW.ORG includes a 30-180 day warranty on every transmission."},
    {q:"What types of transmissions do you carry?",a:"We source automatic, manual, CVT, dual-clutch, and transfer cases for all makes and models from 1990 to present. We match the specific gear count, torque converter specs, and drive configuration to your exact application."},
    {q:"How long does a used transmission last?",a:"A well-maintained used transmission from a low-mileage donor can last 80,000-150,000+ additional miles with proper installation, fresh fluid, and a new filter. The key factors are the mileage and maintenance history of the donor vehicle."}
  ];
  return <>
    ...
    <section className="sec" style={{padding:"48px 24px",background:tok.bg}}>
      <div style={{maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 360px",gap:44,alignItems:"start"}}>
        <div>
          ...
          <div className="gc" style={{padding:24,marginBottom:24}}><GSB/><div style={{position:"relative",zIndex:1}}>
            <h2 style={{fontFamily:MH,fontSize:13,fontWeight:900,color:tok.fg,marginBottom:12,textTransform:"uppercase",letterSpacing:".04em"}}>Welcome to AUAPW.ORG — Used Transmissions</h2>
            <p style={{fontSize:13,color:tok.muted,lineHeight:1.85,fontWeight:300,marginBottom:10}}>You are here because you are looking for a used transmission for your car. There are lots of auto part dealers selling quality used transmissions online. Prices may vary from dealer to dealer. Some dealers will offer a transmission for $1,750 where the same transmission is available for $950. Reaching all the auto parts dealers is not easy, but we have the solution to find quality, cheap used transmissions in just one click.</p>
            <p style={{fontSize:13,color:tok.muted,lineHeight:1.85,fontWeight:300,marginBottom:10}}>Are you looking for an affordable and high-quality car transmission? Transmission is the second most important part of your vehicle after the engine — and it can be a real struggle when it stops working properly. Buying a new one can cost a lot of money, but fortunately we have the perfect alternative — used transmissions from the most trusted dealers in the country.</p>
            <p style={{fontSize:13,color:tok.muted,lineHeight:1.85,fontWeight:300}}>At AUAPW.ORG, you can find A-grade transmissions for all types of vehicles, such as BMW, Honda, Chevy, and more. Whether you need a standard or automatic transmission, you can find the best deals here. With our simple search tool, it will take just a few minutes to find the most affordable and high-quality used transmission.</p>
          </div></div>
          ...
          <h2 style={{fontFamily:MH,fontSize:13,fontWeight:900,color:tok.fg,marginBottom:14,textTransform:"uppercase",letterSpacing:".04em"}}>10 Things to Watch for When Buying a Used Transmission</h2>
          <p style={{fontSize:13,color:tok.muted,lineHeight:1.85,fontWeight:300,marginBottom:20}}>When your car needs a transmission replacement, the first thing that comes to mind is unexpected expenses. But if you can find quality used transmissions, you will be able to save money and make a profit on your purchase. Here are the most important aspects that give the best chance of locating good quality used transmissions for your vehicle.</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:28}}>
            {tenPoints.map(function(p){return(
              <div key={p.n} style={{padding:"14px 16px",background:tok.card,border:"1px solid "+tok.bdd,borderRadius:6,transition:"all .2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,.2)";e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=tok.bdd;e.currentTarget.style.transform="none";}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <div style={{width:24,height:24,borderRadius:6,background:"rgba(255,255,255,.08)",border:"1px solid "+tok.bdd,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:9,fontWeight:900,color:tok.fg,fontFamily:MH}}>{p.n}</div>
                  <div style={{fontSize:11,fontWeight:700,color:tok.fg,fontFamily:MH}}>{p.t}</div>
                </div>
                <div style={{fontSize:11,color:tok.muted,lineHeight:1.65,fontWeight:300}}>{p.d}</div>
              </div>
            );})}
          </div>
          ...
          {sent?(
            ...
            <div style={{position:"relative",zIndex:1}}>
              ...
              <div style={{marginBottom:12}}><div style={{fontSize:9,color:tok.dim,letterSpacing:".18em",textTransform:"uppercase",marginBottom:5,fontFamily:MH}}>Subject</div>
                <select value={subject} onChange={e=>setSubject(e.target.value)} style={{...fs,appearance:"none" as const,cursor:"pointer"}}>
        ...
        {logoSrc&&(
          <div style={{background:"#1a3a1a",borderBottom:"2px solid #4ade80",padding:"8px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,zIndex:9999}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <span style={{fontSize:13,fontWeight:700,color:"#4ade80",fontFamily:MH,letterSpacing:".08em"}}>YOUR LOGO IS APPLIED EVERYWHERE</span>
            </div>
